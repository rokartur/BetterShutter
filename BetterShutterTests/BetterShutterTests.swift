...
struct WindowHairlineTests {
    /// Synthetic "window capture": opaque dark rounded rect centered on a transparent bitmap
    /// (the transparent margin stands in for the shadow area).
    private func makeWindowImage(size: Int = 200, inset: CGFloat = 40, radius: CGFloat = 20) -> CGImage {
        let space = CGColorSpace(name: CGColorSpace.sRGB)!
        let ctx = CGContext(data: nil, width: size, height: size, bitsPerComponent: 8, bytesPerRow: 0,
                            space: space, bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)!
        let body = CGRect(x: 0, y: 0, width: CGFloat(size), height: CGFloat(size))
    ...
    private func pixel(_ image: CGImage, x: Int, y: Int) -> (r: UInt8, g: UInt8, b: UInt8, a: UInt8) {
        ...
        ctx.draw(image, in: CGRect(x: 0, y: 0, width: image.width, height: image.height))
        let p = ctx.data!.bindMemory(to: UInt8.self, capacity: ctx.bytesPerRow * image.height)
        let o = y * ctx.bytesPerRow + x * 4
        return (p[o], p[o + 1], p[o + 2], p[o + 3])
    }

    @Test
    func strokesHairlineAlongOpaqueBodyEdge(){
        let base = makeWindowImage()
        let out = WindowHairline.stroked(on: base, scale: 2)
        #expect(out != nil)
        guard let out else { return }
        #expect(out.width == base.width && out.height == base.height)
        // Left edge midpoint (buffer top-left coords): body spans x 40...159, so the stroke sits
        // just inside x=40. Must be brighter than the untouched dark fill.
        let before = pixel(base, x: 41, y: 100)
        let after = pixel(out, x: 41, y: 100)
        #expect(after.r > before.r + 20)
        // Body interior stays dark, margin stays transparent.
        let interior = pixel(out, x: 100, y: 100)
        #expect(interior.r < 30)
        #expect(pixel(out, x: 10, y: 10).a == 0)
    }

    @Test
    func bailsOnFullyTransparentImage() {
        let space = CGColorSpace(name: CGColorSpace.sRGB)!
        let ctx = CGContext(data: nil, width: 50, height: 50, bitsPerComponent: 8, bytesPerRow: 0,
                            space: space, bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)!
        let empty = ctx.makeImage()!
        #expect(WindowHairline.stroked(on: empty, scale: 2) == nil)
    }
}
